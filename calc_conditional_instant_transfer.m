function instantTransfer = calc_conditional_instant_transfer(x, y, z, order, windowSize)
...
nAllXprevY = zeros(cardX, (cardY^order)*(cardX^order)*(cardZ^(order+1))); %joint distribution of current and previous x and previous y
nPrevXprevY = zeros(1, (cardY^order)*(cardX^order)*(cardZ^(order+1))); %joint distribution of previous x and y
nAllXallY = zeros(cardX, (cardY^(order+1))*(cardX^order)*(cardZ^(order+1))); %joint distribution of current and previous x and y
nPrevXallY = zeros(1, (cardY^(order+1))*(cardX^order)*(cardZ^(order+1))); %joint distribution of previous x and current and previous y
...
for i = order+1:lengthX
    ...
    lastXlastYcurrZstr = lastXstr + lastYstr*(cardX^order) + currZstr*(cardY^order)*(cardX^order);
    lastXcurrYcurrZstr = lastXstr + currYstr*(cardX^order) + currZstr*(cardY^(order+1))*(cardX^order);
    
    % add new symbols to the probability distribution
    prevXprevYstr(i) = lastXlastYcurrZstr + 1;
    prevXallYstr(i) = lastXcurrYcurrZstr + 1;
    ...
    if (i >= order+windowSize) %recompute nX each time
        ...
        instantTransfer(i) = condEntropy(nAllXallY./windowSize, condProbXAllY) - ...
            condEntropy(nAllXprevY./windowSize, condProbXPrevY);
