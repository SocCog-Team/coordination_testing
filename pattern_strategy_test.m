...
for iSeeIndex = 1:nSeeIndex
    ...
    jointPatternDistribution{iSeeIndex}(nStrategy*nRun+1:end, :) = jointPatternDistribution{iSeeIndex}(1:nStrategy*nRun);    
    
    simplifiedFeatures{iSeeIndex} = zeros(2*nStrategy*nRun, 8);
    simplifiedFeatures{iSeeIndex}(:, 1) = jointPatternDistribution{iSeeIndex}(:, 7);   
    simplifiedFeatures{iSeeIndex}(:, 2) = jointPatternDistribution{iSeeIndex}(:, 10);
    simplifiedFeatures{iSeeIndex}(:, 3) = jointPatternDistribution{iSeeIndex}(:, 4);   
    simplifiedFeatures{iSeeIndex}(:, 4) = jointPatternDistribution{iSeeIndex}(:, 13);
    simplifiedFeatures{iSeeIndex}(:, 5) = jointPatternDistribution{iSeeIndex}(:, 14) + jointPatternDistribution{iSeeIndex}(:, 15);
    simplifiedFeatures{iSeeIndex}(:, 6) = jointPatternDistribution{iSeeIndex}(:, 8)  + jointPatternDistribution{iSeeIndex}(:, 12);   
    simplifiedFeatures{iSeeIndex}(:, 7) = jointPatternDistribution{iSeeIndex}(:, 16);
    simplifiedFeaturesInData{iSeeIndex}(:, 8) = jointPatternDistribution{iSeeIndex}(:, 1) + jointPatternDistribution{iSeeIndex}(:, 2) + ...
...
simplifiedFeaturesInData = zeros(nFileTotal, 7);
simplifiedFeaturesInData(:, 1) = jointPatternDistributionInData(:, 7);   
simplifiedFeaturesInData(:, 2) = jointPatternDistributionInData(:, 10);
simplifiedFeaturesInData(:, 3) = jointPatternDistributionInData(:, 4);   
simplifiedFeaturesInData(:, 4) = jointPatternDistributionInData(:, 13);
simplifiedFeaturesInData(:, 5) = jointPatternDistributionInData(:, 14) + jointPatternDistributionInData(:, 15);
simplifiedFeaturesInData(:, 6) = jointPatternDistributionInData(:, 8)  + jointPatternDistributionInData(:, 12);   
simplifiedFeaturesInData(:, 7) = jointPatternDistributionInData(:, 16);
simplifiedFeaturesInData(:, 8) = jointPatternDistributionInData(:, 1) + jointPatternDistributionInData(:, 2) + ...
...
for iSeeIndex = 1:nSeeIndex    
    ...
    for iCluster = 1:nCluster
        ...
        caxis([0 64])
        set( gca, 'fontsize', FontSize, 'YDir', 'normal', 'FontName', fontType, 'XTick', 1:4, 'XTickLabel', {'BB','BR','RB','RR'}, 'YTick', 1:4, 'YTickLabel', {'RR','RB','BR','BB'});
